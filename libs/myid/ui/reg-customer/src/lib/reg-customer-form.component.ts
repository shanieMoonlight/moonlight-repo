...
} from '@spider-baby/ui-kit/inputs';
...
import {
  PasswordValidation,
  PhoneValidation,
} from '@spider-baby/utils-forms/validators';
...
export class SbRegisterCustomerFormComponent {
  ...
  set passwordValidators(validators: ValidatorFn[] | undefined | null) {
    setTimeout(() => {
      if (!validators?.length)
        ...
        this._form.controls.password.setValidators([
          Validators.required,
          ...validators,
        ]);
    }, 500);
  ...
  protected _form: FormGroup<RegisterCustomerForm> =
    this._fb.nonNullable.group<RegisterCustomerForm>(
      {
        ...
        phoneNumber: this._fb.control<string | null>('', [
          PhoneValidation.validator(),
        ]),
        password: this._fb.nonNullable.control<string>('', [
          Validators.required,
          ...PasswordValidation.validationArray(6),
        ]),
  ...
  constructor() {
    this._form.controls.phoneNumber.valueChanges.subscribe((phone) => {
      ...
      const formattedPhone =
        this._phoneFormatter.formatPhoneInternational(phone);
