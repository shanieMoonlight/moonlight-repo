...
import { SbButtonAmazonLoginComponent } from './btn-amazon-login.component';
import { SocialAuthService, SocialUser, AmazonLoginProvider } from '@abacritt/angularx-social-login';

describe('SbButtonAmazonLoginComponent', () => {
  ...
  //--------------------------//
    ...
    it('should instantiate the component in an injection context', () => {
        const component = runInInjectionContext(injector, () => new SbButtonAmazonLoginComponent());
  ...
  //--------------------------//
    ...
    it('login() should call SocialAuthService.signIn and emit the social user', async () => {
        const fakeUser: SocialUser = {
            provider: AmazonLoginProvider.PROVIDER_ID,
            id: '123',
            name: 'Test User',
        ...
        expect(mockAuthService.signIn).toHaveBeenCalledWith(AmazonLoginProvider.PROVIDER_ID);
