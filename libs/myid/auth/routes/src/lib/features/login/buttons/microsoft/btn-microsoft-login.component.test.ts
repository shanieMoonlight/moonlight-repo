...
import { SbButtonMicrosoftLoginComponent } from './btn-microsoft-login.component';
import { SocialAuthService, SocialUser, MicrosoftLoginProvider } from '@abacritt/angularx-social-login';

describe('SbButtonMicrosoftLoginComponent', () => {
  ...
  //--------------------------//
    ...
    it('should instantiate the component in an injection context', () => {
        const component = runInInjectionContext(injector, () => new SbButtonMicrosoftLoginComponent());
  ...
  //--------------------------//
    ...
    it('login() should call SocialAuthService.signIn and emit the social user', async () => {
        ...
        expect(mockAuthService.signIn).toHaveBeenCalledWith(MicrosoftLoginProvider.PROVIDER_ID);
