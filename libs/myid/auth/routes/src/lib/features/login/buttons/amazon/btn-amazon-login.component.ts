import { AmazonLoginProvider, SocialAuthService, SocialUser } from '@abacritt/angularx-social-login';
...
@Component({
  selector: 'sb-button-amazon-login',
  imports: [
  ...
  template: `
      <sb-icon-button         
          [sbTooltip]="showTooltip() ? 'Amazon' : ''"
          aria-label="Amazon Oauth Login"
          (click)="login()"
 ...
 *
 * Button component that initiates a Amazon OAuth sign-in using the injected SocialAuthService.
 * When user clicks the button, it triggers the Amazon login flow.
 * Which when successful, triggers SocialAuthService.authStage with the AmazonUserData which can be subscribed to.
 * 
 *  ngOnInit() {
 ...
 *
 * @remarks
 * - Injects SocialAuthService to perform the sign-in flow for the Amazon provider.
 * - Exposes a `login` method that triggers the provider sign-in.
 * - Exposes inputs to control tooltip visibility and to supply the Amazon SVG icon markup.
 *
 * @example
 * <sb-button-amazon-login 
 *  [showTooltip]="true" 
 *  [faceBookIconSvg]="amazonSvg"/> //Optional
 *
 * @property {SocialAuthService} _socialAuth - (private) Injected SocialAuthService used to perform sign-in operations.
 * @property {() => Promise<any>} login - Initiates sign-in with the Amazon provider (delegates to SocialAuthService.signIn).
 * @property {boolean} showTooltip - Input: when true, a tooltip is shown for the button. Defaults to `true`.
 * @property {string} faceBookIconSvg - Input: SVG markup (or identifier) used to render the Amazon icon. Defaults to `AMAZON_ICON`.
 */
export class SbButtonAmazonLoginComponent {
