...
export abstract class ABaseHttpService {
  ...
  protected _getTimeout<T>(ids: Identifier | Identifier[], timeoutMillis: number = 60000, opts = {}): Observable<T> {
    const url = UrlUtils.combine(this.url, ...this.idsToArray(ids));
    return this._http.get<T>(url, opts ?? {}).pipe(
      map(this.extractData),
  ...
  protected _getActionTimeout<T>(action: string, timeoutMillis: number = 60000, opts = {}): Observable<T> {
    const url = UrlUtils.combine(this.url, action);
    return this._http.get<T>(url, opts ?? {}).pipe(
      map(this.extractData),
      timeout(timeoutMillis),
  ...
  protected _postActionTimeout<T>(action: string, resource: unknown, timeoutMillis: number = 60000, opts = {}): Observable<T> {
    const url = UrlUtils.combine(this.url, action);
  ...
  protected _patchAction<T>(action: string, resource: unknown, opts = {}): Observable<T> {
    const url = UrlUtils.combine(this.url, action);
    return this._http.patch<T>(url, resource, opts).pipe(
      map(this.extractData),
      catchError((error) => this.handleError(error))
  ...
  protected _deleteAction<T>(action: string, ids: Identifier | Identifier[], opts = {}): Observable<T> {
    const deleteUrl = UrlUtils.combine(this.url, action, ...this.idsToArray(ids));
    return this._http.delete<T>(deleteUrl, opts).pipe(
      map(this.extractData),
      catchError((error) => this.handleError(error))
    );
