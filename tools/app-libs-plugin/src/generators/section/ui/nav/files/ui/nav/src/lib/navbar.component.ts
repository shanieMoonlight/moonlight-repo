...
export class <%= classNamePrefix %><%= className %>NavbarComponent {
  ...
  protected _logoSml = signal(<%= classNamePrefix %>AppImages.Logo.small);

  protected _gitRepoUrl = signal(<%= classNamePrefix %>AppConstants.GIT_REP_URL);
  protected _npmPkgUrl = signal(<%= classNamePrefix %>AppConstants.NPM_PKG_URL);
